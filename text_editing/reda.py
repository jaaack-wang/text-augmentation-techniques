...
class REDA:
    ...
    def __init__(self, lang='zh', syn_path=None, tokenizer=None, sep=None):
        ...
        self.sep = sep
        assert self.lang in ['zh', 'en'], 'Parameter lang must be either zh (Chinese) or English (en).'
            
        if not self.syn_path:
            if self.lang == 'zh':
                self.syn_path = 'synonyms_zh.json'
            else:
                self.syn_path = 'synonyms_en.json'
            
        self._syn = json.load(open(self.syn_path))
